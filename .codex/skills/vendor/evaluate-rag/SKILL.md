...
### Evaluating RAG Traces That Become Training Data

When RAG traces are converted into SFT rows, evaluate the dataset as its own
artifact before training. The checks should run on the final serialized rows the
trainer will consume, because repair/export steps can introduce problems that
are invisible in the original trace.

Check citation/source alignment after any source repair, pruning, or reorder.
All concrete claims in the assistant answer should be present in the retrieved
context supplied to the model. Audit the final assistant message for prompt
residue, generator labels, source-pack scaffolding, raw table fragments, clipped
source text, glued citation/code formatting, repeated words, colon labels with
no payload, terminal clipped phrases before citations, raw standalone
identifiers, and orphaned source fragments.

Compare train and eval questions and answers for exact and near-duplicate
leakage. Role prefixes or audience addendums do not make duplicated rows
independent; intentional role-contrast duplicates should be tagged and reviewed
as contrast examples. Validate row length with the target model tokenizer,
because RAG source packs can make rough estimates misleading.
...
- Treating a held-out eval split as trustworthy before checking for exact or near-duplicate leakage from training rows.
- Training on RAG traces after only schema/citation checks, without auditing the final assistant text for residual scaffolding and malformed repairs.
