...
- Custom evidence-grounded SFT datasets need semantic validation as well as format validation: citation/source integrity, source-order stability, concrete-claim visibility, final assistant-text hygiene, train/eval leakage, near-duplicate review, and target-tokenizer sequence length.
...
**Validate before launching GPU training.** Format mismatches are the most common
training failure, and semantic dataset defects are the fastest way to waste a
successful run.
...
- Format validation on CPU costs ~$0.01 and takes <1 minute
- Semantic validation catches rows that are trainer-compatible but harmful to
  learn from
...
- Evidence-grounded SFT datasets, even when the JSON schema is valid

**Skip format validation for known TRL datasets:**
- `trl-lib/ultrachat_200k`, `trl-lib/Capybara`, `HuggingFaceH4/ultrachat_200k`, etc.

Do not skip semantic validation for local/custom evidence-grounded SFT rows. The
dataset inspector proves the trainer can read the data; it does not prove the
rows are worth training on.

### Evidence-Grounded SFT Validation

Before launching a training job on cited RAG rows, validate the final serialized
artifact that the trainer will consume:

- Verify every `[S#]` citation maps to the supplied source for that row, and
  prune or justify unused sources.
- Preserve source-order integrity. If source order or membership changes during
  repair, remap citations or regenerate the assistant answer.
- Check that dates, counts, versions, URLs, project names, role titles, and
  other concrete identifiers in the answer are visible in the source context
  supplied to the model.
- Audit final assistant text for generator residue, prompt fragments, source-pack
  tables, template labels, raw dumps, clipped fragments, malformed citation/code
  spacing, repeated words, colon labels without payload, terminal clipped phrases
  before citations, raw standalone identifiers, orphaned source fragments, and
  repair phrases.
- Check exact duplicates and near-duplicates across train/eval splits; role
  prefixes or audience addendums are still leakage when the answer is copied.
- Validate final row length with the target model tokenizer and intended maximum
  sequence length.
- Review gold rows and the highest-overlap train/eval pairs before submitting a
  GPU job.
...
9. **Validate dataset format and semantic quality** before training with dataset inspector plus any task-specific gates, especially evidence-grounded SFT citation/source checks, leakage checks, and target-tokenizer length checks
