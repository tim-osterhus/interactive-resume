...
- [ ] **Backup script exists**
  ```bash
  ls -lh /path/to/project/backend/scripts/backup_golden_dataset.py
...
- [ ] **Data directory exists**
  ```bash
  mkdir -p /path/to/project/backend/data
  ls -ld /path/to/project/backend/data
...
- [ ] **Check backup file created**
  ```bash
  ls -lh /path/to/project/backend/data/golden_dataset_backup.json
  # Expected: ~2.5 MB file
  ```

- [ ] **Check metadata file created**
  ```bash
  ls -lh /path/to/project/backend/data/golden_dataset_metadata.json
  # Expected: ~1 KB file
  ```
...
- [ ] **Verify backup file is valid JSON**
  ```bash
  cat /path/to/project/backend/data/golden_dataset_backup.json | jq '.'
  # Expected: Valid JSON, no parse errors
  ```

- [ ] **Check backup version**
  ```bash
  cat /path/to/project/backend/data/golden_dataset_backup.json | \
    jq '.version'
  # Expected: "2.0"
  ```
...
- [ ] **OpenAI API key set** (for embedding regeneration)
  ...
  # Expected: OPENAI_API_KEY=<OPENAI_API_KEY>
  ```

- [ ] **Sufficient disk space**
  ```bash
  df -h /path/to/project/backend/data
...
- [ ] **Verify fixture files restored** (v2.0 backups only)
  ```bash
  ls -lh /path/to/project/backend/tests/smoke/retrieval/fixtures/
  # Expected:
...
- [ ] **Check for SQL dump**
  ```bash
  ls -lh /path/to/project/backend/data/golden_dataset_dump.sql
...
- [ ] **Import SQL dump**
  ...
  psql -h localhost -p 5437 -U orchestkit < \
    /path/to/project/backend/data/golden_dataset_dump.sql
  ```
