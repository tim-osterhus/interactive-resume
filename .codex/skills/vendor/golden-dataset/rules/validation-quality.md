...
| Citation Integrity | Cited answer IDs reference supplied sources | Error |
| Source Order Integrity | Reordered/pruned sources have remapped or regenerated citations | Error |
| Concrete Claim Visibility | Numeric, date, version, URL, title, and role claims appear in supplied context | Error |
| Final Assistant Text Hygiene | No prompt residue, generator labels, raw dumps, clipped fragments, or repair phrases | Error |
| Train/Eval Leakage | Exact duplicates are blocked and near-duplicates are reviewed or rewritten | Error |
| Target Tokenizer Length | Rows fit the intended model tokenizer and sequence length | Error |
| Content Quality | Title/content length, tag count | Warning |
| Difficulty Distribution | Balanced query difficulty levels | Warning |
| Intentional Contrast Tags | Duplicate contrast questions are explicitly tagged and justified | Warning |

### Evidence-Grounded SFT Row Checks

For fine-tuning rows that include source excerpts and cited answers, validate the
final serialized training artifact, not only the source worksheet. Every `[S#]`
citation must point to a supplied source; unused sources should be pruned unless
they are intentionally included as context. If a repair step changes source order
or source membership, remap citations or regenerate the answer before export.

The answer itself must be audited as user-visible assistant text. Reject rows
with prompt instructions, source-pack scaffolding, table residue, generator
labels, repeated words, clipped fragments, colon labels without payload,
terminal clipped phrases before citations, raw standalone identifiers, orphaned
source fragments, or repair phrases. Concrete facts
such as dates, counts, versions, URLs, project names, and role titles must be
visible in the supplied context.

Do not rely on role prefixes to make duplicated answers safe. If an eval answer
is just a train answer with "For hiring review" or "In diligence terms" added,
rewrite it. Intentional role-contrast duplicate questions should be tagged so
the duplicate checker can distinguish them from leakage.
