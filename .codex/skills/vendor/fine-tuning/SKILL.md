...
| Live HF job logs | `python tuner.py cloud-jobs logs --job <hf-user>/<job-id> --tail 200` |
...
Before starting SFT, KTO, or GRPO, validate the exact dataset artifact that the
trainer will read. Do not rely on an upstream spreadsheet, draft export, or
rough token estimate.

For all training methods, confirm schema compatibility, required fields,
split/balance expectations, exact duplicates, train/eval leakage, and target
model tokenizer length against the configured sequence length.

For evidence-grounded SFT, also require citation/source integrity, source-order
integrity after any repair or pruning, concrete-claim visibility in the supplied
prompt, and assistant-text hygiene. The final assistant message should not
contain prompt scaffolding, generator labels, raw source-pack fragments, glued
citation formatting, repeated words, clipped fragments, colon-only labels,
terminal clipped phrases before citations, raw standalone identifiers, or repair phrases.
High-overlap train/eval pairs and gold rows should receive human or senior-model
...
- Run the pre-training dataset gate on the final exported artifact before any training launch.
...
Use [`.skills/research-reporting/SKILL.md`](/path/to/training-project/.skills/research-reporting/SKILL.md) alongside this skill whenever the user wants experiment tracking, research summaries, or reusable post-run analysis.
...
python tuner.py cloud-jobs show --job <hf-user>/<job-id>
python tuner.py cloud-jobs logs --job <hf-user>/<job-id> --tail 200
```
Gotcha:
...
python3 scripts/prune_dataset_from_loss.py \
  ...
  --publish-repo <hf-user>/<dataset-name>
```
